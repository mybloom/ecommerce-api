...
UseCase 컨벤션과 동일하게 `from(input)`이 한 입력으로 변환 가능할 때만 사용한다 (@docs/conventions/usecase.md "from()이 동작하지 않는 경우" 참고).
...
UseCase 테스트는 도메인 흐름을 따라가는 게 원칙이지만 (@docs/conventions/usecase.md "픽스처와 셋업 패턴" 참고), E2E는 트레이드오프가 다르다.
