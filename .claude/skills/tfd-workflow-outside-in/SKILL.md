...
5. **아래 레이어는 계약만 만든다.** 지금 레이어가 부르는 대상의 **시그니처와 DTO만** 만들고
   본문은 `UnsupportedOperationException` 스켈레톤이나 목으로 막는다. 동작은 다음 단계에서 채운다.
6. **최소한만 구현한다.** 테스트가 통과하는 가장 작은 코드만. 추측성 기능 추가 금지.
...
- **중간 단계에서는 전체 스위트가 초록이 아닐 수 있다.** 아래가 스켈레톤이라 관통 경로가 끊겨 있다.
  단계별 테스트로 확인하고, 전체는 마지막 단계에서 맞춘다.
...
1. **UseCase 테스트 작성**
    - 이 단계에서는 도메인 서비스가 아직 스켈레톤이므로 **목으로 대체**한다
    - 정상 흐름, 비즈니스 예외 시나리오(`CoreException`), 트랜잭션 경계가 갈리는 지점
...
### 진행 순서
1. **1단계에서 만든 E2E 테스트의 `@Disabled`를 뗀다**
2. **E2E 통과 확인**
3. **스켈레톤 잔존 전수 확인**
   ```bash
...
### 완료 조건
- [ ] E2E 테스트에 `@Disabled`가 남아 있지 않고 전부 통과
