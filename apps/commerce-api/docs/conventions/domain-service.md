...
> 트랜잭션 어노테이션(`@Transactional`) 적용 정책은 도메인이 더 쌓이면 정리한다.
> 일반 가이드: 상태를 변경하는 메서드는 `@Transactional`, 조회 전용은 `@Transactional(readOnly = true)`.
...
상세 패턴은 @docs/conventions/testing.md "Mock 기반 서비스 테스트" 참고.
