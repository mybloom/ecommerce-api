...
## 2. Given-When-Then을 빈 줄로 표현

별도 주석(`// given`, `// when`, `// then`)을 달지 않고 빈 줄로 구분한다. 짧은 테스트에서 주석은 시각적 노이즈가 된다.
...
## 4. `assertAll`은 같은 객체의 여러 측면을 볼 때만

`assertAll`은 한 객체의 여러 속성을 동시에 검증할 때 유용하다. 여러 입력 케이스를 묶는 용도로 쓰지 않는다.
