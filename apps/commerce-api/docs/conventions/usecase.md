...
여러 도메인 서비스를 조합해 하나의 비즈니스 유스케이스를 구현하고, 트랜잭션 경계를 책임진다. 도메인 객체와 도메인 서비스는 도메인 레이어에 있고(@docs/conventions/domain-service.md), UseCase는 그것들을 호출자(컨트롤러 등) 입장의 흐름으로 엮는 곳이다.
...
`from`은 **정적 팩토리** 패턴(@docs/conventions/domain-modeling.md 참고). 생성자가 아닌 정적 메서드로 변환을 표현해 의도를 명확히 한다.
...
도메인 레이어와 동일한 원칙을 따른다 (@docs/conventions/domain-modeling.md "예외 타입 구분" 참고).
